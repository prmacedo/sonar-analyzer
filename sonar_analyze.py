...
def run_analysis(project_dir, project_key, sonar_host, sonar_token):
    ...
    if is_flutter_project(project_dir):
        ...
        cmd = build_standard_scanner_command(
            scanner, project_dir, project_key, sonar_host
        )
    ...
    # Keep the token out of the process command line (and therefore out of ps).
    scanner_env["SONAR_TOKEN"] = sonar_token
    result = subprocess.run(cmd, capture_output=True, text=True, env=scanner_env)
...
def build_standard_scanner_command(scanner, project_dir, project_key, sonar_host):
    """Build the scanner command for non-Flutter projects."""
    project_dir_norm = project_dir.replace("\\", "/")
    ...
    configured_binaries = os.getenv("SONAR_JAVA_BINARIES")
    if configured_binaries:
        cmd.append(f"-Dsonar.java.binaries={configured_binaries}")
    else:
        project_path = os.path.abspath(project_dir)
        binary_candidates = [
            os.path.join(project_path, "target", "classes"),
            os.path.join(project_path, "backend", "target", "classes"),
            os.path.join(project_path, "build", "classes", "java", "main"),
            os.path.join(project_path, "backend", "build", "classes", "java", "main"),
        ]
        existing_binaries = [path for path in binary_candidates if os.path.isdir(path)]
        if existing_binaries:
            cmd.append(f"-Dsonar.java.binaries={','.join(existing_binaries)}")

    return cmd


_CE_TASK_ID = re.compile(r"/api/ce/task\?id=([A-Za-z0-9_-]+)")


def extract_ce_task_id(scanner_output):
    """Extract the Compute Engine task ID printed by sonar-scanner."""
    match = _CE_TASK_ID.search(scanner_output or "")
    return match.group(1) if match else None


def wait_for_ce_task(sonar_host, sonar_token, task_id, timeout=300, interval=3):
    """Wait until SonarQube finishes processing a submitted analysis."""
    url = f"{sonar_host.rstrip('/')}/api/ce/task"
    ...
    while True:
        ...
        if status in {"FAILED", "CANCELED"}:
            ...
            raise RuntimeError(f"SonarQube analysis task failed: {message}")
        if time.monotonic() >= deadline:
            raise TimeoutError(f"Timed out waiting for SonarQube task {task_id}")
...
def save_to_csv(output_dir, project_key, username, metrics, data):
    ...
    with open(filename, "w", newline="") as csvfile:
        ...
        for metric in metrics:
            writer.writerow(
                [username, project_key, now, metric, measures_by_metric.get(metric) or "0"]
            )
...
def main():
    ...
    scanner_output = run_analysis(project_dir, project_key, args.sonar_host, args.sonar_token)
    task_id = extract_ce_task_id(scanner_output)
    if task_id:
        print(f"[Sonar] Waiting for server processing (task {task_id})...")
        try:
            wait_for_ce_task(
                args.sonar_host,
                args.sonar_token,
                task_id,
                timeout=int(os.getenv("SONAR_CE_TIMEOUT", "300")),
            )
        except (requests.RequestException, RuntimeError, TimeoutError) as error:
            print(f"❌ Could not retrieve completed SonarQube analysis: {error}")
            sys.exit(1)
    else:
        print("⚠️ Scanner did not report a Compute Engine task; fetching measures now.")
